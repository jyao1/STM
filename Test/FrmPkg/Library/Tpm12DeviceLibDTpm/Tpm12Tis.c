  ...
  )
{
  ...
  MmioWrite8((UINTN)&TisReg->Access, TIS_PC_ACC_ACTIVE);
  Status = Tpm12TisPcWaitRegisterBits (
             &TisReg->Access,
             TIS_PC_VALID,
             TIS_PC_ACC_ACTIVE,
...
**/
EFI_STATUS
Tpm12PtpCrbRelinquishTpm (
  IN      PTP_CRB_REGISTERS_PTR      CrbReg
  )
{
  EFI_STATUS                        Status;
  
  MmioWrite32((UINTN)&CrbReg->LocalityControl, PTP_CRB_LOCALITY_CONTROL_RELINQUISH);
  Status = Tpm12PtpCrbWaitRegisterBits (
             &CrbReg->LocalityStatus,
             0,
             PTP_CRB_LOCALITY_STATUS_GRANTED,
  ...
  )
{
  ...
  switch (PtpInterface) {
  case PtpInterfaceCrb:
    return Tpm12PtpCrbRelinquishTpm ((PTP_CRB_REGISTERS_PTR) (UINTN) PcdGet64 (PcdTpmBaseAddress));
  case PtpInterfaceFifo:
  case PtpInterfaceTis:
    return Tpm12TisPcRelinquishTpm ((TIS_PC_REGISTERS_PTR) (UINTN) PcdGet64 (PcdTpmBaseAddress));
  default:
    return EFI_NOT_FOUND;
  }
}
