  ...
  )
{
  ...
  Psd = (TXT_PROCESSOR_SMM_DESCRIPTOR*)(mCpuHotPlugData.SmBase[CpuIndex] + SMM_PSD_OFFSET);
  SmiMtrrs = (UINT64*)(UINTN)Psd->Reserved6; // MtrrBaseMaskPtr
  ...
  )
{
  ...
  StmExceptionStacks = NULL;
  if (FeaturePcdGet(PcdCpuStmSupport)) {
    //
    // Allocate exception stacks
    //
    DEBUG((EFI_D_ERROR, "NumberOfCpus - %x\n", mMaxNumberOfCpus));
    DEBUG((EFI_D_ERROR, "CpuSmmStmExceptionStackSize - %x\n", PcdGet32(PcdCpuSmmStmExceptionStackSize)));
    DEBUG((EFI_D_ERROR, "Pages - %x\n", EFI_SIZE_TO_PAGES(mMaxNumberOfCpus * PcdGet32(PcdCpuSmmStmExceptionStackSize))));
    StmExceptionStacks = AllocatePages(EFI_SIZE_TO_PAGES(mMaxNumberOfCpus * PcdGet32(PcdCpuSmmStmExceptionStackSize)));
    ASSERT(StmExceptionStacks != NULL);
  }

  GdtTssTables = InitGdt (Cr3, &GdtTableStepSize);

  //
  // Initialize TXT_PROCESSOR_SMM_DESCRIPTOR for each CPU
  //
  for (Index = 0; Index < mMaxNumberOfCpus; Index++) {
    ...
    if (FeaturePcdGet (PcdCpuStmSupport)) {
      ...
      Psd->SmmSmiHandlerRip = mCpuHotPlugData.SmBase[Index] + GetSmiHandlerOffset ();
      Psd->SmmSmiHandlerRsp = gSmiStack;

      Psd->SmmCr3 = gSmiCr3;

      Psd->StmProtectionExceptionHandler.SpeRsp = (UINT64)(UINTN)StmExceptionStacks + PcdGet32 (PcdCpuSmmStmExceptionStackSize) * (Index + 1);
      Psd->BiosHwResourceRequirementsPtr = (UINT64)(UINTN)GetStmResource ();
      Psd->LocalApicId = (UINT32)mCpuHotPlugData.ApicId[Index];
      Psd->AcpiRsdp = 0;
      ...
      Psd->PhysicalAddressBits = 36; // BUGBUG
    }
  }
