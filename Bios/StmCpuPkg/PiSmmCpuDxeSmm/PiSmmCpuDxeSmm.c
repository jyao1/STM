...
//    AsmWriteMsr64 (IA32_SMM_MONITOR_CTL_MSR_INDEX, MsegBase);
    // BUGBUG: Some processor can not write 0x9B twice
    AsmWriteMsr64 (IA32_SMM_MONITOR_CTL_MSR_INDEX, MsegBase | IA32_SMM_MONITOR_VALID);
  }
}
  ...
  )
{
  if (FeaturePcdGet (PcdCpuStmSupport)) {
    ...
    DEBUG ((EFI_D_INFO, "SmmEndOfDxeEventNotify\n"));
  ...
  )
{
  ...
  if (FeaturePcdGet (PcdCpuStmSupport)) {
    ...
    if (GuidHob != NULL) {
      ...
      Buffer = (VOID *)(UINTN)SmramDescriptor->CpuStart;

      mMsegBase = (UINT32)(UINTN)Buffer;

      //
      // Write 0xFFFFFFFF to the header to mark it is invalid.
      //
      *(UINT32 *)Buffer = 0xFFFFFFFF;
      DEBUG ((EFI_D_ERROR, "MsegBase: 0x%08x, MsegSize: 0x%08x\n", (UINTN)mMsegBase, (UINTN)SmramDescriptor->PhysicalSize));
    }
    //
    // Alloccate SMRAM memory range for MSEG if MSEG size is not zero
    //
    else if (PcdGet32 (PcdCpuMsegSize) != 0) {
      //
      // Currently we assume the last SMRAM range (T-SEG typically) is larger
      // than the MSEG requested in size, for simplicity.
      //
      Buffer = AllocatePages (EFI_SIZE_TO_PAGES (PcdGet32 (PcdCpuMsegSize)));
      ASSERT (Buffer != NULL);
      ...
      //
      *(UINT32 *)Buffer = 0xFFFFFFFF;
      DEBUG ((EFI_D_ERROR, "MsegBase (Allocated): 0x%08x, MsegSize: 0x%08x\n", (UINTN)mMsegBase, (UINTN)PcdGet32 (PcdCpuMsegSize)));
    }
  }
  ...
  // specific context in a TXT_PROCESSOR_SMM_DESCRIPTOR, and the SMI entry point.  This size
  // is rounded up to nearest power of 2.
  //
  TileCodeSize = GetSmiHandlerSize ();
  TileCodeSize = ALIGN_VALUE(TileCodeSize, SIZE_4KB);
  TileDataSize = sizeof (SMRAM_SAVE_STATE_MAP) + sizeof (TXT_PROCESSOR_SMM_DESCRIPTOR);
  ...
  // the TXT_PROCESSOR_SMM_DESCRIPTOR of CPU[i+1] and the SMRAM Save State Map of CPU[i+1],
